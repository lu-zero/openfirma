...
- `HakoniwaBackend::prepare` now threads `request.profile.mounts` through as operator-provided `SandboxMount`s (mirroring `BwrapBackend::prepare`'s first mount-population step only — not its DNS-stub-driven resolv.conf mounts, since Slice 3 covers those separately. Identity-mode-driven passwd/group mounts remain out of scope: this was originally written expecting a later slice to add them, but neither Slice 2 nor Slice 4 did, and no slice currently scopes identity-mode support — see `hakoniwa/mod.rs`'s `HakoniwaLaunchContract` doc comment for the corrected, up-to-date status of this gap).
