...
- `HakoniwaBackend::prepare` now threads `request.profile.mounts` through as operator-provided `SandboxMount`s (mirroring `BwrapBackend::prepare`'s first mount-population step only — not its DNS-stub-driven resolv.conf mounts, since Slice 3 covers those separately). Identity-mode support landed in a later session, via a different mechanism than mounts — see the finding below.
...
- Intentionally still unsupported: `tests/e2e/scenarios/child_process_governance/filesystem.rs` is not yet parametrized over `Hakoniwa` (`DEC-011`'s parametrize-don't-fork approach) — deferred to whichever slice actually needs it exercised (Slice 6 at the latest); no CA trust injection port (unchanged from the original scope note; still unverified whether it needs anything beyond ordinary file mounts). `tests/e2e/scenarios/config_masking.rs`, by contrast, now IS parametrized across `{Bwrap, Hakoniwa}` (done in a later session) — the first place this repo's own "parametrize, don't fork" approach was actually implemented, not just named as the goal.
- **A fourth, later-session finding, in two parts: identity-mode was first blocked, then landed via a different mechanism than the one attempted; resolv.conf parity is still blocked.** Attempting to mirror `BwrapBackend::prepare`'s `SandboxUser` passwd/group mounts and resolv.conf stub-mounting for `HakoniwaBackend` verbatim fails live: `firma-hakoniwa-runner`'s `container.rootfs("/")` call reuses the host's real `/bin`/`/etc`/`/usr`/etc. directories as read-only bind mounts wholesale (`hakoniwa::Container::rootfs`'s own `rootfs_imp`) rather than building a fresh, selectively-populated rootfs the way `bwrap` does. Overlaying our own content at `/etc/passwd`, `/etc/group`, or `/etc/resolv.conf` — paths that already exist as real, root-owned host files through that baseline mount — fails with `touch("etc/group") => Permission denied`, because the `hakoniwa` crate's own mount-target-creation step (`sys::touch`/`sys::fwrite`) opens the _real_ target file for write/append rather than creating a placeholder in its own staging area. This broke every `SandboxUser`-identity-mode Hakoniwa launch (the config-masking e2e suite's own default), not just an edge case — reverted before landing.
  - **Identity-mode: landed anyway, via a mechanism that sidesteps the mount problem entirely.** `hakoniwa::Container` already unshares a `User` namespace with an identity uid/gid mapping by default (`Container::new()`); overriding it with `Container::uidmap`/`gidmap` to the standard `nobody`/`nogroup` ids (`65534`) is a genuine kernel-level remap — no mount, no file overlay, nothing for `rootfs("/")`'s baseline `/etc` bind-mount to conflict with. `getuid()` inside the sandbox reports `65534`, `getpwuid`-based lookups (`whoami`) resolve it against the real host `/etc/passwd` to the universal `nobody` account (not the real username, and not an error), and `$USER`/`$LOGNAME` are separately set to `firma-user` for tools that check the env var instead. Verified live end to end. Notably, this is arguably _more_ correct than `bwrap`'s own current implementation: `bwrap` never passes `--uid`/`--gid` at all, so `SandboxUser` mode there is purely cosmetic (`/etc/passwd` content + env vars, same real uid throughout) — Hakoniwa's version is the first backend to actually deliver the "isolated uid" the CLI help text (`crates/firma/src/args/run.rs`) already claimed for both.
  - **Resolv.conf: still blocked, no equivalent alternate mechanism found.** Unlike identity-mode, there's no namespace-level substitute for overlaying `/etc/resolv.conf`'s _content_ — this remains an open gap. Two real findings survive regardless: `/etc/resolv.conf`'s real contents (nameserver IPs, search domain) are currently readable from inside a Hakoniwa sandbox (network-namespace isolation still blocks any real query those nameservers would need, so this is an information leak, not a confinement bypass), and ordinary resolver-based DNS lookups (`getaddrinfo`) never reach the sandbox's own DNS-refusal stub at all — only a query hard-coded to `127.0.0.1:53` does. Closing this needs `/etc` reconstructed as a fresh, writable layer (e.g. a tmpfs mounted at `/etc` ahead of these files in Hakoniwa's own target-path mount ordering) without silently dropping real host `/etc` content (locale data, `ca-certificates`, `nsswitch.conf`) many programs need — a design decision for a future slice, not scoped here. **Superseded by `docs/architecture/hakoniwa-etc-reconstruction-plan.md`** (Accepted), which resolves this gap: a `Tmpfs` anchor at `/etc` plus a curated, hardcoded preserved-path allowlist, with `/etc/resolv.conf`/`/etc/hosts` synthesized rather than preserved.
...
**Partially implemented.** `firma doctor`'s `Backend::Hakoniwa` arm and its own `check_hakoniwa` (`crates/firma/src/doctor/sandbox.rs`) are done, matching the plan's own framing exactly: a kernel unprivileged-userns capability check reusing `HostProbe`'s existing `wsl`/`userns_restricted` fields, not a `Prober`/`--version` CLI probe, since `hakoniwa` is an embedded library reached via `firma-hakoniwa-runner`, not a `$PATH` tool. Writing this check surfaced a real, previously-undiscovered gap it needed to stay consistent with — `HakoniwaBackend::prepare()` had no WSL check at all (`cfg!(target_os = "linux")` is true under WSL), now fixed via a new `preflight_host_support` function in `hakoniwa/mod.rs` mirroring `linux_bwrap`'s function of the same name. The `Hakoniwa` vs. `Bwrap` launch/exec overhead benchmark is separately satisfied by `openfirma-notes/bench/RESULTS.md`'s existing `PtraceSeccompExec`/steady-state numbers (see `selectable-execution-governance-plan.md`'s Slice 4 notes). Still not done: the `docs-site/src/content/docs/concepts/sandbox.md` update, and the full `child_process_governance` suite is not yet green against `Hakoniwa` as a parametrized target — `execution.rs` today is a single fixed-config (`Inherited`-only) test, not a suite selectable per backend/strategy.
