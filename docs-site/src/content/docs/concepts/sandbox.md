...
5. **Identity remap.** The agent runs under a sandbox user (configurable via `--identity-mode`). The two backends implement this differently: `bwrap` presents a cosmetic `firma-user` identity (`/etc/passwd`/`/etc/group` content, `$USER`/`$LOGNAME`) without changing the real uid at all — no `--uid`/`--gid` flag is passed. `hakoniwa` does a genuine kernel-level remap instead (`Container::uidmap`/`gidmap` to the standard `nobody`/`nogroup` ids), alongside the same cosmetic `$USER`/`$LOGNAME` env vars — `getuid()` and `whoami` inside a Hakoniwa sandbox report `nobody`, not the real host uid or username.
...
9. **`/etc` reconstruction (`hakoniwa` only).** `hakoniwa`'s sandbox setup reuses the host's real `/etc` wholesale by default. `firma run` replaces it with a fresh, empty layer, then re-populates a reviewed allowlist of real host paths read-only (`nsswitch.conf`, dynamic-linker cache/config, timezone data, `passwd`/`group`, `services`/`protocols`) and synthesizes `/etc/resolv.conf` (pointing at the sandbox's own DNS-refusal stub) and `/etc/hosts` (a minimal, loopback-only stub). Before this, the real host's nameserver IPs/search domain were readable from inside the sandbox, and a real static `/etc/hosts` entry could resolve a name without ever reaching the DNS stub — both closed by this reconstruction. `/etc/machine-id`, `/etc/hostname`, `/etc/ssl`, and `/etc/pki` are deliberately not re-exposed. `bwrap` does not need this — it never bind-mounts the whole host `/etc` in the first place.
...
| DNS confinement | **Yes.** Sandbox resolver points at the local DNS stub or fails closed. | **Yes.** Same DNS-stub shape; the stub's sockets are bound and fd-inherited before capability drop, closing a privileged-port bind failure `bwrap`'s own DNS stub still has open. `/etc/resolv.conf`/`/etc/hosts` are synthesized as part of the `/etc` reconstruction above, so ordinary resolver-based lookups reach the stub too, not just a query hand-crafted to `127.0.0.1:53`. | **Planned.** Guest-local deterministic resolver target. |
...
| Identity remap | **Cosmetic.** `/etc/passwd`/`/etc/group` content + `$USER`/`$LOGNAME` present a `firma-user` identity; the real uid is unchanged (no `--uid`/`--gid` flag). | **Real.** `Container::uidmap`/`gidmap` to `nobody`/`nogroup` — a genuine kernel-level remap, plus the same cosmetic `$USER`/`$LOGNAME` env vars. | **Planned.** |
...
- An agent trivially fingerprinting the real host account it's running under (`whoami`, `$USER`, `getpwuid`) — both backends present a non-identifying identity (`bwrap`: cosmetic `firma-user`; `hakoniwa`: a real uid remap resolving to `nobody`). This is a confidentiality mitigation, not a filesystem access-control boundary: identity remap does not by itself restrict what files the agent can read or write on either backend — that protection comes entirely from the `.firma` config mask and mount-plan design described above, independent of identity mode.
